...
(*    | Exp.Exn _ -> 
  ...
  let handle_unknown_call pdesc ret callee_pname params node (mem,conds) = 
    match Procname.get_method callee_pname with
      "malloc" -> (model_malloc pdesc ret callee_pname params node mem, conds)
    | _ ->
      (match ret with 
         Some (id, _) -> 
          let ret_loc = Loc.of_var (Var.of_pvar (Pvar.get_ret_pvar callee_pname)) in
          (Domain.Mem.add ret_loc Domain.Val.top_itv mem , conds)
       | None -> (mem, conds))
