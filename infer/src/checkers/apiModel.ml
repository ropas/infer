...
type ret_typ =
  ...
  | AllocConst of Dom.Val.t (* New block, filled with given abstract val. *)
...
|> ApiMap.add "getenv" {arg_typs = [Skip]; ret_typ = AllocBuf}
...
(*
|> ApiMap.add "catgets" {arg_typs = [Skip; Skip]; ret_typ = clean_ret}
|> ApiMap.add "dlerror" {arg_typs = []; ret_typ = clean_ret}
|> ApiMap.add "GWEN_I18N_Translate" {arg_typs = [Skip; Skip]; ret_typ = clean_ret}
|> ApiMap.add "frexp" {arg_typs = [src; Skip]; ret_typ = src_ret}
|> ApiMap.add "ldexp" {arg_typs = [src; Skip]; ret_typ = src_ret}
|> ApiMap.add "tgetnum" {arg_typs = [src]; ret_typ = src_ret}
|> ApiMap.add "tgetflag" {arg_typs = [Skip]; ret_typ = clean_ret}
|> ApiMap.add "argz_next" {arg_typs = [src; Skip; Skip]; ret_typ = src_ret}
*)
(* Maybe UNSOUND *)
(*
|> ApiMap.add "tgetstr" {arg_typs = [src; dst]; ret_typ = src_ret}
|> ApiMap.add "memmem" {arg_typs = [src; Skip; Skip; Skip]; ret_typ = src_ret}
|> ApiMap.add "wcwidth" {arg_typs = [Skip]; ret_typ = clean_ret}
|> ApiMap.add "tgoto" {arg_typs = [src; Skip; Skip]; ret_typ = src_ret}
|> ApiMap.add "png_get_image_width" {arg_typs = [Skip; Skip]; ret_typ = clean_ret}
*)
