...
struct
 ...
 and eval_unop : Unop.t -> Exp.t -> Mem.astate -> Location.t -> Val.t
    ...
    | Exp.UnOp (Unop.LNot, Exp.Var x, _) ->
        (match Mem.find_alias x mem with
         | Some x' ->
             ...
             in
             let v' =
               (itv_v, Val.get_taint v, Val.get_pow_loc v, Val.get_array_blk v)
             in
             Mem.update_mem (PowLoc.singleton lv) v' mem
         | None -> mem)
    | _ -> mem
  ...
  let get_taint_match tenv formal actual typ caller_mem callee_mem =
    let get_taint v = Val.get_taint v in
    let add_pair_val v1 v2 acc =
      let t1 = get_taint v1 in
      let t2 = get_taint v2 in
      assert (FSTaintSet.cardinal t2 = 1);
      let t = FSTaintSet.choose t2 in
      (t, t1) :: acc
    in
    []
    |> add_pair_val formal actual
  ...
  let get_subst
    ...
    let add_pair (formal, typ) actual acc =
      let formal = Mem.find_heap (Loc.of_pvar formal) callee_m in
      let new_matching =
